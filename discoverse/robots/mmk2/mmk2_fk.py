...
class MMK2FK:
    def __init__(self, mjcf_path=None):
        if mjcf_path is None:
            ...
            xml_text = src.read_text().replace("__REPO_ROOT__", str(task_dir))
            # The V2 client image ships MuJoCo 3.2.x.  The exported ArUco
            # marker mesh uses newer/flat-mesh features that 3.2.x rejects
            # while loading.  FK only needs robot kinematic site transforms, so
            # removing these visual-only marker meshes from the temporary FK
            # model is safe and keeps the detector compatible with the official
            # client image.
            xml_text = xml_text.replace('inertia="shell"', "")
            xml_text = re.sub(
                r'\s*<mesh\s+name="aruco_marker_3cm_mesh"[\s\S]*?/>',
                "",
                xml_text,
                count=1,
            )
            xml_text = re.sub(
                r'\s*<geom[^>\n]*mesh="aruco_marker_3cm_mesh"[^>]*/>',
                "",
                xml_text,
            )
            runtime.write_text(xml_text)
